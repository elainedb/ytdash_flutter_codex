...
class VideosLocalDataSourceImpl implements VideosLocalDataSource {
  ...
  Future<Database> _initDb() async {
    ...
    final path = p.join(databasesPath, 'videos.db');
    return openDatabase(
      path,
      version: 1,
  ...
  Future<List<VideoModel>> getVideosByChannel(String channelName) async {
    try {
      ...
      throw CacheException('Failed to read videos by channel: $error');
    }
  }
  ...
  Future<List<VideoModel>> getVideosByCountry(String country) async {
    try {
      ...
      throw CacheException('Failed to read videos by country: $error');
    }
  }
